# this displays the folder path and name to the right of the header in the search, so each item occupies only one line. better use of space, but remains to be seen how long headers are handled.
import sublime
...
class GotoMarkdownSymbolPureCommand(sublime_plugin.WindowCommand):
    ...
    def _show_panel(self, items):
        if not items:
            ...
            return
        self.items = items
        # QuickPanelItem's annotation renders at the right edge of the
        # SAME row as the trigger - no extra vertical line needed - and,
        # like a detail line, it is display-only and not searched. This
        # is the one combination the plain string-list item format can't
        # do: compact single-row entries where part of the row is
        # excluded from matching.
        display_items = [
            sublime.QuickPanelItem(trigger=title_line, annotation=rel_path)
            for title_line, rel_path, _, _ in items
        ]
...
class GotoSymbolSwitchToFilenameCommand(sublime_plugin.WindowCommand):
    """
    Meant to be bound with an 'overlay_visible' context so it only fires
    while this quick panel (or any quick panel) is open. Closes the
    current panel and opens native Goto Anything in filename mode -
    the equivalent of switching from symbol search to Ctrl+P mid-flow.

    Note: whatever you'd already typed does NOT carry over. Sublime's
    API doesn't expose the quick panel's live query text to plugins, so
    there's no way to hand it off - the new panel opens blank.
    """
