...
class GotoMarkdownSymbolCommand(sublime_plugin.WindowCommand):
    ...
    (your own edits), and GotoMarkdownSymbolReindexCommand (manual, for
...
class GotoMarkdownSymbolReindexCommand(sublime_plugin.WindowCommand):
    """
...
class GotoMarkdownSymbolWarmCacheListener(sublime_plugin.EventListener):
    ...
    def on_activated_async(self, view):
        ...
        if not folders:
            return
        now = time.monotonic()
        if now - GotoMarkdownSymbolWarmCacheListener._last_focus_refresh < self._FOCUS_REFRESH_COOLDOWN:
            return
        GotoMarkdownSymbolWarmCacheListener._last_focus_refresh = now
        threading.Thread(target=_refresh_cache, args=(folders,)).start()